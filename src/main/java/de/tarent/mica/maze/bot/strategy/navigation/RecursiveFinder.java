...
/**
 * This class is responsible for finding a route from a start position to a
 * destination.
 *
 * @author rainu
 */
public class RecursiveFinder {
	...
	public RecursiveFinder(Maze maze, Coord start, Coord dest, TimeUnit unit,
			long maxDuration) {
		this.maze = maze;
		this.start = start;
		this.dest = dest;
	...
	public RecursiveFinder(Maze maze, Coord start, Coord dest) {
	...
	private List<Coord> getNeighbors(Coord curCoord, Coord dest) {
		...
		for (Direction dir : directions) {
			switch (dir) {
			case NORTH:
				neighbors.add(curCoord.north());
				break;
			case EAST:
				neighbors.add(curCoord.east());
				break;
			case SOUTH:
				neighbors.add(curCoord.south());
				break;
			case WEST:
				neighbors.add(curCoord.west());
				break;
			}
		}
	...
	// TODO: Vorher das Labyrinth analysieren... je nach "bauart" ist eine
	// "rekursionsrichtung" besser geeignet
	private List<Direction> getShortestDirections(Coord curCord, Coord dest) {
		List<Direction> result = new ArrayList<Direction>(4);

		int horizontalDist = Math.max(curCord.getX(), dest.getX())
				- Math.min(curCord.getX(), dest.getX());
		int verticalDist = Math.max(curCord.getY(), dest.getY())
				- Math.min(curCord.getY(), dest.getY());
		...
		if (!result.contains(Direction.NORTH))
			result.add(Direction.NORTH);
		if (!result.contains(Direction.EAST))
			result.add(Direction.EAST);
		if (!result.contains(Direction.SOUTH))
			result.add(Direction.SOUTH);
		if (!result.contains(Direction.WEST))
			result.add(Direction.WEST);
